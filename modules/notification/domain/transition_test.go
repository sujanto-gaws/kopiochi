...
// statuses — 25 of them — not only the six legal moves.
//
// The forbidden pairs are the point. A legal-moves-only test passes just as
// happily against a Transition that accepts everything, and the failure mode it
// misses is the expensive one: a row that goes sent -> pending is delivered
// twice, and a row that goes dead -> sending is retried after the operator
// declared it undeliverable.
...
func TestTransitionRejectsUnknownStatus(t *testing.T) {
	...
	if err := n.Transition(StatusSending); !errors.Is(err, ErrInvalidTransition) {
		t.Fatalf("from unknown status: got %v, want ErrInvalidTransition", err)
	}

	n = &Notification{Status: StatusPending}
	if err := n.Transition(bogus); !errors.Is(err, ErrInvalidTransition) {
...
func TestTransitionErrorNamesBothEnds(t *testing.T) {
	...
	err := n.Transition(StatusPending)
	if err == nil {
