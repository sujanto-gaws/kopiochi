...
| `pquerna/otp` | identity MFA (`modules/identity/infrastructure/mfa/totp.go`) | **Yes** — the module is wired |
| `boombuler/barcode` | *nothing* | **No importer anywhere** |
...
`docs/docs.go` (338 lines), `docs/swagger.json`, and `docs/swagger.yaml` are
generated by `make swagger-docs`. Committing them creates review noise and merge
conflicts.

> *An earlier revision cited this file as `docs/api/docs.go` at 2,100 LOC. No
> `docs/api/` directory appears in any commit; the real file is `docs/docs.go` and
> it is 338 lines. Corrected, not withdrawn — the recommendation is unchanged, but
> the weight argument for it is much weaker than stated.*
...
1. Delete the unreachable FIDO2 plugin and the extension frameworks (including
   `internal/extension/identity/`); drop `go-webauthn`, `go-tpm`, and
   `fxamacker/cbor` from `go.mod`.
2. Drop `boombuler/barcode` — nothing imports it. Keep `pquerna/otp`; the live
   identity module uses it for TOTP.
